...
class GameCornerController extends Controller
{
    ...
    {
        $user = Auth::user();
        $gameCornerPokemon = GameCornerPokemon::find($id);

        if ($user->casino_coins < $gameCornerPokemon->cost) {
            return redirect()->back()->with(['return' => $gameCornerPokemon->cost - $user->casino_coins]);
        }

        if ($gameCornerPokemon->pNum == 0) {
            $pokemon = $this->buyRandomShadowPokemon($gameCornerPokemon);
        } else {
            $pokemon = new Pokemon([
                'save_id' => $user->selectedSave()->id,
                'pId' => $this->getUniquePokemonId(Save::whereId($user->selectedSave()->id)->first()->pokemon()),
                'pNum' => $gameCornerPokemon->pNum,
                'nickname' => $gameCornerPokemon->nickname,
                'exp' => $gameCornerPokemon->exp,
                'lvl' => $gameCornerPokemon->lvl,
                'm1' => $gameCornerPokemon->m1,
                'm2' => $gameCornerPokemon->m2,
                'm3' => $gameCornerPokemon->m3,
                'm4' => $gameCornerPokemon->m4,
                'ability' => $gameCornerPokemon->ability,
                'mSel' => $gameCornerPokemon->mSel,
                'targetType' => $gameCornerPokemon->targetType,
                'tag' => 'n',
                'item' => 0,
                'owner' => 0,
                'pos' => 1,
                'shiny' => (mt_rand(1, 100) <= 1) ? 1 : $gameCornerPokemon->shiny,
            ]);

            DB::transaction(function () use ($user, $gameCornerPokemon, $pokemon) {
                $user->casino_coins -= $gameCornerPokemon->cost;
                $user->save();
        ...
        return redirect()->back()->with(['game-corner-pokemon' => $pokemon]);
    }

    public function buyRandomShadowPokemon($gameCornerPokemon)
    {
        if (empty($gameCornerPokemon)) {
            return redirect()->back();
        }
        ...
        DB::transaction(function () use ($pokemon, $gameCornerPokemon, $pNum, $user) {
            $user->casino_coins -= $gameCornerPokemon->cost;
            $user->save();
